<template>
	<div
		:id="`${props.from}_${props.issue.UUID}`"
		:draggable="props.draggable"
		class="card"
		@dragstart="dragStart"
	>
		<button @click="toggle(`issue-content-${props.issue.UUID}`)">
			{{ props.issue.SequenceNumber }}
		</button>
		<div :id="`issue-content-${props.issue.UUID}`" class="issue-content">
			<button @click="toggle(`description-${props.issue.UUID}`)">
				Description
			</button>
...
button {
	@apply block text-left px-2 py-1 my-1 w-full;
}
</style>
